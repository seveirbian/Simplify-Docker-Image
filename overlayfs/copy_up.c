...
static int ovl_copy_up_inode(struct ovl_copy_up_ctx *c, struct dentry *temp)
{
	...
	if (S_ISREG(c->stat.mode)) {
		...
		//得到upperdir目录的挂载点和upperdir目录中的dentry
...
static int ovl_do_copy_up(struct ovl_copy_up_ctx *c)
{
	...
	if (S_ISREG(c->stat.mode) && ofs->tmpfile) {
		...
		//处理普通非临时文件，即主要需要修改的地方

		//确保workdir和upperdir的合法性，并加锁
			   ...
			   int flags)
{
	...
	//获取dentry的lowerdir挂载点和lowerdirdentry
	...
	if (parent) {
		...
		//获取upperdir挂载点以及parent的对应的upperdir目录下的dentry
...
int ovl_copy_up_flags(struct dentry *dentry, int flags)
{
	...
	while (!err) {
		...
		//判断当前dentry是否有upper对应的dentry以及（确认已经copy up了或者该dentry无效？）
		//ovl_dentry_has_upper_alias()用于确认已经copy up，
		//因为对于硬链接和decoded file handles，ovl_dentry_upper()不能够正确反应
		if (ovl_dentry_upper(dentry) &&
